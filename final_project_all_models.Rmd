...
aov1 <-
  aov(
    subject_age ~ arrest_made + time_of_day + district + subject_race + subject_sex + 
      officer_assignment + type + contraband_found + frisk_performed + search_person + 
...
# Produce confusion matrix from prediction and data used for training
confusionMatrix(pred_cv_knn_cl, train.knn_cl$trainingData$.outcome, mode = "everything")
```
...
# Produce confusion matrix from prediction and data used for training
confusionMatrix(pred_cv_cart_cl, train.cart_cl$trainingData$.outcome, mode = "everything")
```
...
```{r}
train.ols.sex <- lm(subject_age ~ . + subject_sex * ., data = orleans_train)
...
# Predict
pred_cv_ols_interaction <- predict(train.ols.interaction, orleans_valid)
...
train.ridge <- 
    train(
      ...
      method = "glmnet",
      trControl = ctrl,
      # tuneGrid = expand.grid(alpha = 0, lambda = seq(0.01, 2, by = 0.1))
    )
...
```{r}
ctrl_none <- trainControl(method = "none", number = 10, savePredictions = TRUE)
...
# Predict
#pred_cv_rmse <- predict(train.forest, train_cleaned)
